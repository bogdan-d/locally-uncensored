...
export function ParamGroups() {
  ...
  return (
    <div className="py-1">
      ...
      >
        <Slider label="Steps" min={1} max={60} step={1} value={s.steps} onChange={s.setSteps} />
        <Slider label={isVideo ? 'Guidance' : 'CFG scale'} min={0} max={30} step={0.5} value={s.cfgScale} onChange={s.setCfgScale} format={(v) => v.toFixed(1)} />
      ...
      <Section title="Output" icon={Boxes} defaultOpen>
        ...
        {!isVideo && <Slider label="Batch size" min={1} max={8} step={1} value={s.batchSize} onChange={s.setBatchSize} />}
        {isVideo && (
          <div className="grid grid-cols-2 gap-2">
