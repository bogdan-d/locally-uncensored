...
Drop-in hotfix on top of v2.4.4. Five user-reported bugs collected from Discord, Reddit, and GitHub Issues / Discussions between v2.4.4-release (2026-05-11) and 2026-05-17.
...
- **Bug D**: Linux webkit env-var function unit-tested cross-platform via 5 cargo tests (sets-both-when-unset, preserves user DMABUF override, preserves user COMPOSITING override, preserves empty-string as explicit unset, idempotent). Arch VirtualBox boot test deferred — env-var workaround is the documented upstream-recommended fix for the symptom class.
- **Bug E**: PEP 668 detection + venv creation unit-tested cross-platform via 13 cargo tests across `install.rs::tests` and `python.rs::tests`. Path-layout tests cover both Windows (`venv/Scripts/python.exe`) and Unix (`venv/bin/python`) layouts. Resolver tests use real tempdirs to verify the `Some(path)` vs `None` gating. Diagnose tests cover Arch's exact `error: externally-managed-environment` string from rzgrozt's report plus the shorter `error: externally-managed` variant and confirm the suggested distro-install commands (`pacman -S python-virtualenv`, `apt install python3-venv`, `dnf install python3-virtualenv`) appear in the user-facing hint. Live Arch boot test deferred — same constraint as Bug D (no Arch box on dev machine); the venv approach is the standard answer to PEP 668 (used by every Python tool that touches Arch / Debian 12 / Fedora 38) so the structural-correctness bar is very different from a hardware-driven bug.
